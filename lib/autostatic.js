...
AutoStatic.prototype.middleware = function() {
  ...
  return function(req, res, next) {
    ...
    req.on('static', function(stream) {
      ...
      var ver = self.manager.getEtag(p);
      if (self.checkRemote) Manager.remoteHas(p);
...
Object.defineProperty(AutoStatic.prototype, 'serve', {
  get: function(p) {
    ...
    return function(p) {
      if (self.debug) return self.direct + p;
      ...
      var isRemoteReady = !self.checkRemote || Manager.remoteHas(uri);

      if (isRemoteReady) return uri + ver;

      // if local is ready, serve the minified version
      var isLocalReady = !self.checkLocal || manager.isReady(p);

      if (isLocalReady) return self.direct + manager.getMin(p) + ver;
...
AutoStatic.prototype.set = function setOption(options) {
  ...
  if ('watch' in options) manager.doWatch == options.watch;
};
