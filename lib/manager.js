...
Manager.prototype.getEtag = function getEtag(p) {
  ...
  fs.stat(filepath, function(err, st) {
    if (err || !st) {
      console.error('[autostatic] Error when stat file: ', p, err);
      return;
    }
    ...
    if (self.oldEtags[p] !== etag) {
      console.log('[autostatic] Found new version of static file:', p);
...
Manager.prototype.minify = function minify(p, fn) {
  ...
  process.nextTick(function() {
    fs.readFile(filepath, function(err, buffer) {
      if (err) {
        console.error('[autostatic] Error when reading file', p, err);
        return;
      }
      ...
      if (ft == 'css' || ft == 'js') {
        ...
        fs.writeFile(self.dir + self.getMin(p), buffer, function(err) {
          if (err) {
            console.error('[autostatic] Error when write minified version of ', p, err);
          } else {
            self.markReady(p);
          }
        });
      ...
      if (self.checkHash) {
        hash = self.etags[p] = makeHash(buffer);
      ...
      if (res.statusCode == 200) {
        ...
        if (checkHash) {
          ...
          res.on('end', function() {
            ...
            if (result) {
              console.log(uri, remote_hash, '- match -', etag);
            } else {
              console.warn(uri, remote_hash, '- MISS MATCH -', etag);
            }
