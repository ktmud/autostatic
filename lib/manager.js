...
Manager.prototype.checkLocal = true;
Manager.prototype.getEtag = function getEtag(p) {
  var self = this;
  var filepath = path.join(self.dir, p);
  ...
  process.nextTick(function() {
    fs.readFile(filepath, function(err, buffer) {
      ...
      if (ft == 'css' || ft == 'js') {
        ...
        // write to minified
        fs.writeFile(path.join(self.dir, m_p), buffer, function(err) {
          if (err) {
      ...
      if (self.upload) {
        ...
        self.upload(m_p, buffer, function() {
...
Manager.prototype.isReady = function(p) {
  return this.checkLocal ? this.readies[p] : true;
};
...
Manager.prototype.isRemoteReady = function(p, etag, fn) {
  ...
  if (self.checkRemote === false) return true;
  ...
  if (!(uri in remote_readies)) remote_readies[uri] = 0;
