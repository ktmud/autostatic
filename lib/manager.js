...
Manager.prototype.getEtag = function getEtag(p) {
  ...
  fs.stat(filepath, function(err, st) {
    ...
    if (self.oldEtags[p] !== etag) {
      debug_log('[autostatic] Found new version of static file:', p);
      ...
      if (res.statusCode == 200) {
        ...
        if (checkHash) {
          ...
          res.on('end', function() {
            ...
            if (result) {
              debug_log(uri, remote_hash, '- match -', etag);
            } else {
              debug_log(uri, remote_hash, '- MISS MATCH -', etag);
            }
